...
> 本文资料根据http://thesecretlivesofdata.com/raft/整理而成，截图均源于此网站。

# Raft算法是什么？

> 分布式环境下解决数据一致性问题(consensus)协议。
>
> *Raft* is a protocol for implementing distributed consensus.
>
> 真实环境中，分布式下必存在多台服务器，每一台服务器(独立进程，可以在同一台物理机上)叫一个节点(Node)。
...
> 系统启动后，每个节点都是Follower节点。

![image-20200627072800077](images/raft算法/image-20200627072800077.png)

## 2-候选节点

> 当一个集群中没有Leader节点时，即Leader和Followers之间没有心跳，过一段时间后Followers会变成Candidate角色，准备发起投票。具体时间一般是固定间隔(比如10ms)加一个随机数。保证不同节点发起投票的时间不一致。从而防止投票失败。
...
然后主节点向从节点发起提交指令命令。最后，所以节点的的值均为5.
...
如果上图所示，正常启动之后，会有两个计时器控制着选举：**选举超时(election timeout)**。
...
假如节点C一直没收到Leader的心跳，那就在election timeout后，角色即为candidate,准备发起投票。此时，节点A和B还是倒计时。因为election timeout是随机的，总有一个节点会先结束，改变角色，发起新的投票。此的Term会加1。即新的一轮投票，并且自己先投自己一票，然后向节点A和B要票。
